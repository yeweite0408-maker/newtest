    ...
    <div class="page-container" style="max-width:720px;margin:0 auto;">
      ...
      <div v-if="article" class="article-detail">
        <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;align-items:center">
          ...
          <span style="color:#86868b;font-size:12px">{{ article.createdAt?.substring(0, 10) }}</span>
          <span style="color:#86868b;font-size:12px">👁 {{ article.viewsCount || 0 }}</span>
          <span style="color:#86868b;font-size:12px">❤ {{ article.likesCount || 0 }}</span>
        </div>
        <h1>{{ article.title }}</h1>

        <div style="display:flex;align-items:center;gap:12px;margin-bottom:28px;padding:12px 0;border-bottom:1px solid var(--border-color)">
        ...
        <div style="display:flex;gap:12px;margin:24px 0;align-items:center">
          ...
          <span style="font-size:12px;color:#86868b">点赞后可到个人主页查看收藏的文章</span>
        </div>
...
import { getArticle, addView, toggleLike, getComments, addComment } from '../api'
...
async function handleLike() {
  ...
  try {
    ...
    article.value.likesCount += (liked.value ? 1 : -1)
...
.markdown-body blockquote { border-left: 4px solid #0071e3; padding-left: 16px; color: #86868b; margin: 1em 0; }
